...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `${OPENMRS_ROOT_URL}/ws/rest/v1/ghana/reports/nhis-vs-cash?date=${encodeURIComponent(date)}${loc ? `&locationUuid=${encodeURIComponent(loc)}` : ''}${format ? `&format=${encodeURIComponent(format)}` : ''}`;
