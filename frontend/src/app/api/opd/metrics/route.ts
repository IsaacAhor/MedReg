...
export async function GET() {
  ...
  try {
    ...
    const url = `${OPENMRS_ROOT_URL}/ws/rest/v1/ghana/opd/metrics?encounterTypeUuid=${encodeURIComponent(OPD_ENCOUNTER_TYPE_UUID)}${loc ? `&locationUuid=${encodeURIComponent(loc)}` : ''}`;
